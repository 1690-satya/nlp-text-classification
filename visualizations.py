...
def plot_confusion_matrices(y_test, baseline_preds, transformer_preds, 
                           baseline_acc, transformer_acc, save_path=None):
    ...
    sns.heatmap(cm_baseline, annot=True, fmt='d', cmap='Blues', ax=axes[0],
                ...
                cbar=False)
    axes[0].set_title(f'TF-IDF + Logistic Regression\nAccuracy: {baseline_acc:.3f}',
    ...
    sns.heatmap(cm_transformer, annot=True, fmt='d', cmap='Greens', ax=axes[1],
                ...
                yticklabels=['Negative', 'Positive'],
                cbar=False)
    axes[1].set_title(f'DistilBERT\nAccuracy: {transformer_acc:.3f}',
                     fontsize=12, fontweight='bold')
...
def plot_metrics_comparison(y_test, baseline_preds, transformer_preds,
    ...
    accuracy_scores = [baseline_acc, transformer_acc]
    precision_scores = [
        precision_score(y_test, baseline_preds, average='macro'),
        precision_score(y_test, transformer_preds, average='macro')
    ]
    recall_scores = [
        recall_score(y_test, baseline_preds, average='macro'),
        recall_score(y_test, transformer_preds, average='macro')
    ]
    f1_scores = [
        f1_score(y_test, baseline_preds, average='macro'),
        f1_score(y_test, transformer_preds, average='macro')
    ]
    ...
    bars2 = ax.bar(x - 0.5*width, precision_scores, width, label='Precision', color='#2ecc71')
    bars3 = ax.bar(x + 0.5*width, recall_scores, width, label='Recall', color='#e74c3c')
    bars4 = ax.bar(x + 1.5*width, f1_scores, width, label='F1-Score', color='#9b59b6')
    ...
    print("=" * 60)
    print(f"{'Metric':<15} {'TF-IDF + LR':>15} {'DistilBERT':>15}")
    print("-" * 60)
